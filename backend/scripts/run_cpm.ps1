...
try {
    ...
    Send-TgAlert "[VBoard TS FAIL] run_cpm.ps1 exception: $($_.Exception.Message) @ $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
    exit 1
}

if ($exitCode -ne 0) {
    Send-TgAlert "[VBoard TS FAIL] run_cpm.ps1 exit=$exitCode @ $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
}
