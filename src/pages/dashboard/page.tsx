...
import { savedChannels, trendingAlerts } from '@/mocks/userDashboard';
  ...
  const quickActions = [
    {
      ...
      titleKey: 'dash_quick_rankings',
      descKey: 'dash_quick_rankings_desc',
      path: '/rankings',
      color: 'text-red-500 dark:text-red-400',
      bg: 'bg-red-50 dark:bg-red-500/10',
    },
    {
      icon: 'ri-video-line',
      titleKey: 'dash_quick_video',
      descKey: 'dash_quick_video_desc',
      path: '/video-rankings',
    ...
    {
      icon: 'ri-search-line',
      titleKey: 'dash_quick_search',
      descKey: 'dash_quick_search_desc',
      path: '/search',
      color: 'text-cyan-600 dark:text-cyan-400',
      bg: 'bg-cyan-50 dark:bg-cyan-500/10',
    },
  ...
  return (
    <DashboardLayout>
      <div className="px-4 lg:px-6 py-6 space-y-6">
        ...
        <div className="relative overflow-hidden bg-gradient-to-r from-red-50 via-orange-50 to-white dark:from-[#1a0a0a] dark:via-[#1f1010] dark:to-[#0f0f0f] border border-red-100 dark:border-red-900/30 rounded-lg px-6 py-5">
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <Link
                to="/rankings"
                className="px-4 py-2 bg-white dark:bg-white/10 border border-gray-200 dark:border-white/25 text-gray-700 dark:text-white font-medium text-sm rounded-md hover:bg-gray-50 dark:hover:bg-white/18 transition-colors cursor-pointer whitespace-nowrap"
              >
                {t('dash_browse_rankings')}
              </Link>
              <Link
                to="/search"
                className="px-4 py-2 bg-red-600 text-white text-sm rounded-md hover:bg-red-700 transition-colors cursor-pointer whitespace-nowrap"
              >
                {t('dash_search_channels')}
              </Link>
            </div>
          </div>
        </div>
        ...
        {/* Two-column: Recent Searches + Trending Alerts */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {quickActions.map((action) => (
            <Link
              key={action.titleKey}
              to={action.path}
              className="bg-white dark:bg-[#1c1c1c] border border-gray-200 dark:border-white/15 rounded-lg px-5 py-4 hover:border-gray-300 dark:hover:border-white/25 transition-colors cursor-pointer flex items-start gap-3"
            >
