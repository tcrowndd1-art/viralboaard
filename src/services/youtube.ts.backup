...
export async function fetchViralVideos(regionCode = 'KR', maxResults = 50): Promise<ViralVideoItem[]> {
  ...
  // Fetch popular videos with stats in one call
  const videoData = await get<any>('videos', {
    part: 'snippet,statistics',
    chart: 'mostPopular',
    regionCode,
    maxResults: String(maxResults),
  ...
  // Get subscriber counts for each unique channel
  const channelIds = [...new Set(items.map((v: any) => v.snippet?.channelId).filter(Boolean))];
  const chData = await get<any>('channels', {
    part: 'snippet,statistics',
    id: channelIds.join(','),
  });
  ...
  // viralScore = views / max(subscribers, 1)  — higher = blowing up relative to audience size
  const viral = items.map((v: any) => {
    const views = parseInt(v.statistics?.viewCount ?? '0');
    const subscribers = subMap.get(v.snippet?.channelId) ?? 1;
    return {
      rank: 0,
      videoId: v.id,
      title: v.snippet?.title ?? '',
      ...
      subscribers,
      views,
      viralScore: views / Math.max(subscribers, 1),
      uploadDate: v.snippet?.publishedAt?.slice(0, 10) ?? '',
      thumbnail: `https://img.youtube.com/vi/${v.id}/mqdefault.jpg`,
  ...
  return viral.map((v, i) => ({ ...v, rank: i + 1 }));
}
